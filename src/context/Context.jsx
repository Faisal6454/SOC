//Context.js
import React, { createContext, useContext, useState } from "react";

const Context = createContext();

export const useDataContext = () => {
  return useContext(Context);
};

export const DataProvider = ({ children }) => {
  const [viewImage, setViewImage] = useState(1);
  const [modalIsOpen, setModalIsOpen] = useState(false);
  const [filterValue, setFilterValue] = useState("");
  const [filterType, setFilterType] = useState("positive"); // Default value
  const [currentPage, setCurrentPage] = useState(1);
  const imagesPerPage = 12; // Number of images per page
  const [images, setImages] = useState([
    {
image:{
imageUrl: 'https://cdn.pixabay.com/photo/2024/02/25/10/11/forsythia-8595521_1280.jpg',
name:'test image - 1',
},
colorCode: '#0f3443',
index:0
  },
  {
    image:{
    imageUrl: 'data:image/jpeg;base64,',
    name:'test image - 2',
    },
    colorCode: '#0f3443',
    index:0
  },
  {
    image:{
    imageUrl: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS3oo0tHTZ0hgXIfdUjC7TIeTOCXhUpvRBd3g&s',
    name:'test image - 3',
    },
    colorCode: '#0f3443',
    index:0
  },
  {
    image:{
    imageUrl: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS7PETXe5dLZODtdN9AUafllEpnmUORAitGbg&s',
    name:'test image - 4',
    },
    colorCode: '#0f3443',
    index:0
  },
  {
    image:{
    imageUrl: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQkAhvc-YjdwsimlEYdOknxcvbgNOVSHWjkWQ&s',
    name:'test image - 5',
    },
    colorCode: '#0f3443',
    index:0
  },
  {
    image:{
    imageUrl: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRn_aWGnxIOWeg7iqhZAh1UaUiQxQx8VvIV7w&s',
    name:'test image - 6',
    },
    colorCode: '#0f3443',
    index:0
  },
  {
    image:{
    imageUrl: 'https://i.pinimg.com/originals/8b/33/0c/8b330cb1e7f88202b73eaf4d5b3429cb.jpg',
    name:'test image - 7',
    },
    colorCode: '#0f3443',
    index:0
  },
  {
    image:{
    imageUrl: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTnnnObTCNg1QJoEd9Krwl3kSUnPYTZrxb5Ig&s',
    name:'test image - 8',
    },
    colorCode: '#0f3443',
    index:0
  },
  {
    image:{
    imageUrl: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSk3DtYsz71TtEHHOD_4g_QdMDwOM5CLuSLhw&s',
    name:'test image - 9',
    },
    colorCode: '#0f3443',
    index:0
  },
  {
    image:{
    imageUrl: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS75ebrwvgVW5Ks_oLfCbG8Httf3_9g-Ynl_Q&s',
    name:'test image - 10',
    },
    colorCode: '#0f3443',
    index:0
  },
  {
    image:{
    imageUrl: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQEtf9nWhjerKUtrnmDPIKPKQpKU6Vsa8EFag&s',
    name:'test image - 11',
    },
    colorCode: '#0f3443',
    index:0
  },
  {
    image:{
    imageUrl: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTsGMmQnFHk_4n3gT4DapfusuqBuiDtN3GqAQ&s',
    name:'test image - 12',
    },
    colorCode: '#0f3443',
    index:0
  },
  {
    image:{
      imageUrl: 'https://img.freepik.com/premium-photo/full-shot-queer-students-outdoors_23-2150405216.jpg?t=st=1714535856~exp=1714536456~hmac=f24ef83a0f254c1d7a2c9261272b564fab5d26094b0c97999e4b69468723a1602x.webp',
      name:'test image - 13',
      },
      colorCode: '#0f3443',
      index:0
      },
      {
        image: {
        imageUrl: 'https://img.freepik.com/free-photo/photorealistic-portrait-non-traditional-family-structure_23-2151295382.jpg?size=626&ext=jpg',
        name:'test image - 14',
      },
      colorCode: '#0f3443',
      index:0
      },
      {
        image: {
        imageUrl: 'https://img.freepik.com/free-photo/happy-mothers-day-celebration_23-2151306234.jpg?size=626&ext=jpg',
        name:'test image - 15',
      },
      colorCode: '#0f3443',
      index:0
      },
      {
        image: {
        imageUrl: 'https://img.freepik.com/free-photo/vibrant-portrait-person-bright-environment_23-2151078856.jpg?size=626&ext=jpg',
        name:'test image - 16',
      },
      colorCode: '#0f3443',
      index:0
      },

]);

  const onUpload = (newImages) => {
    console.log("onUpload", newImages);
    handleUpload(newImages);
    setModalIsOpen(false);
  };

  const handleFilterTypeChange = (e) => {
    setFilterType(e.target.value);
  };

  const indexOfLastImage = currentPage * imagesPerPage;
  const indexOfFirstImage = indexOfLastImage - imagesPerPage;
  const currentImages = images.slice(indexOfFirstImage, indexOfLastImage);
  const totalPages = Math.ceil(images.length / imagesPerPage);

  const paginate = (pageNumber) => setCurrentPage(pageNumber);


  

  const handleUpload = (newImages) => {
    const updatedImages = [...images, ...newImages];
    console.log("updatedImages", updatedImages);
    setImages(updatedImages);
  };

  return (
    <Context.Provider
      value={{ viewImage, setViewImage, images, setImages, handleUpload 
      ,modalIsOpen,setModalIsOpen,filterValue,setFilterValue,filterType,
      setFilterType,currentPage,setCurrentPage,imagesPerPage,onUpload,
      handleFilterTypeChange,indexOfFirstImage,indexOfLastImage,currentImages,
      totalPages,paginate}}
    >
      {children}
    </Context.Provider>
  );
};
